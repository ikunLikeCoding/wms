...
// 检查name的唯一性
const validateAuthName = async(rule, authName, callback) => {
  authName = authName.trim();
  if(authName === '') return callback(new Error('请输入权限名称！'));
  get(`/auth/auth-name-verify?authName=${authName}`).then(res => {
    if(!res.data){
...
// 检查url的唯一性
const validateAuthUrl = async(rule, authUrl, callback) => {
  authUrl = authUrl.trim();
  if(authUrl === '') return callback(new Error('请输入权限url！'));
  get(`/auth/auth-url-verify?authUrl=${authUrl}`).then(res => {
    if(!res.data){
      callback(new Error('该权限url已存在！'));
    }else{
      callback();
    }
  })
...
// 检查code的唯一性
const validateAuthCode = async(rule, authCode, callback) => {
  authCode = authCode.trim();
  if(authCode === '') return callback(new Error('请输入权限code！'));
  get(`/auth/auth-code-verify?authCode=${authCode}`).then(res => {
    if(!res.data){
      callback(new Error('该权限code已存在！'));
    }else{
      callback();
    }
  })
...
const rules = reactive({
  authName: [ { validator: validateAuthName, trigger: 'blur' } ],
  authUrl: [ { validator: validateAuthUrl, trigger: 'blur' } ],

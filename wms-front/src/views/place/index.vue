...
// 导出数据
const export2Table = () => {
  get("/place/exportTable", params).then(result => {
    // 要导出的数据
    const placeList = result.data;
    const columns = [
      {"title": "产地ID", "key": "placeId"},
      {"title": "产地名称", "key": "placeName"},
      {"title": "产地编号", "key": "placeNum"},
      {"title": "产地描述", "key": "introduce"},
    ];
    export2excel(columns, placeList, "产地信息表");
  });
}

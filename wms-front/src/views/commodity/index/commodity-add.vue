  ...
  <el-dialog v-model="visible" title="添加商品" width="50%" @close="close" destroy-on-close>
    <el-form ref="commodityAddRef" :model="commodityAdd" :rules="rules" label-position="right" label-width="100px">
      <el-form-item label="商品图片：" prop="imgs">
        <!-- :action="`${$apiBaseUrl}/product/img-upload`" -->
        <el-upload
        ...
        <!-- 上传文件 -->
        <!-- <el-upload :http-request="handleFileUpload">
...
import { post, tip, API_BASE_URL } from "@/common"
...
const handleAvatarUpload = uploadFile => {
  ...
  post('/product/img-upload', formData).then(res => {
    // 图片在服务器的名称
...
/* const handleFileUpload = uploadFile => {
  const formData = new FormData();
  formData.append("file", uploadFile.file);
  post('/product/file-upload', formData).then(res => {
